...
2. Node JS (install all lib to code backend: npm install express cors sequelize pg pg-hstore --save)
...
6. Lấy dữ liệu đổ vào frondend
    ...
    1. Proxy
        ...
        add proxy in file package.json of frontend: "proxy":"http://localhost:4000/" để cho phép truy cập back end
        edit path axios: 
            
            const getProductData = () => axios.get('/getdata01').then((res) => res.data);
            
        bỏ: http://localhost:4000/
        
    2. Backend config request:
    
        // for parsing application/json
        app.use(bodyParser.json()) => use body raw(JSON/aplication/json) with post method
